...
$spName=$args[6]
write-host $spName
$folderWithintegration-serviceInstaller=$args[7]
Write-Host $folderWithintegration-serviceInstaller
$appRegisterTimestamp=$args[8]
write-host $appRegisterTimestamp
...
""
""
Write-Host "Generated and unique blobName based on app register timestamp:"
$blobName = $blobName+'_'+$appRegisterTimestamp
$blobName
""
""
""
#####################################################
#  Storage Acccount already created by 06-createStorageQueue.ps1 in an earlier Stage
#
...
" "
" "
Write-Host "Content of folder with integration-service installer: $folderWithintegration-serviceInstaller"
ls -l $folderWithintegration-serviceInstaller
""
""
""
write-host "show azure storage container system properties and metadata:"
az storage container show --name $storageContainerName --account-name $storageAccount --connection-string $storageAccountConnectionString 
" "
" "
" "
write-host "Show storage account blob-service properties:"
az storage account blob-service-properties show --account-name $storageAccount --resource-group $resourceGroup
" "
" "
" "
...
az storage blob upload --account-name $storageAccount --container-name $storageContainerName --name $blobName --file $folderWithintegration-serviceInstaller\integration-service-OnPremise_installer.exe --connection-string $storageAccountConnectionString --overwrite true
#--overwrite true  # in preview and under development
" "
" "
" "
#############################################################################################################################
# DETECTED ISSUE: .exe filename extension is not recognized by windows when integration-service installer is downloaded from this storage blob.
#                 This issue does not arise with MacOS.
#                 Content-Disposition header in response is not sent when the download URL is not authenticated. 
# SOLUTION: Setup Content-Disposition header.
#           Azure Blob with CONTENT-DISPOSITION="attachment;filename=integration-service.exe"
#
# References about Azure Storage API ContentDisposition:
# https://stackoverflow.com/questions/20719641/azure-storage-api-contentdisposition
# https://stackoverflow.com/questions/54207583/azure-blob-storage-v2-does-not-deliver-content-disposition-header-any-more
# https://docs.microsoft.com/en-us/rest/api/storageservices/set-blob-service-properties
# The Set Blob Service Properties operation sets properties for a storage account’s Blob service endpoint, 
# including properties for Storage Analytics, CORS (Cross-Origin Resource Sharing) rules and soft delete settings.
#############################################################################################################################
Write-Host "Sets system properties on the blob:"
az storage blob update --container-name $storageContainerName --connection-string $storageAccountConnectionString --name $blobName --content-disposition "attachment;filename=integration-service.exe" 
" "
" "
" "
write-host "Show all properties of a blob:"
az storage blob show --account-name $storageAccount --connection-string $storageAccountConnectionString --container-name $storageContainerName --name $blobName
" "
" "
" "
...
Write-Host "setup output variable needed by the next stage: BlobFileDownloadUrlOutput"
Write-Host "##vso[task.setvariable variable=BlobFileDownloadUrlOutput;isOutput=true]$url1"

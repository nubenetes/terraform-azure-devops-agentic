...
MYDBNAME=$2
MYENV=$3
echo "MongoDB Connection: $MONGODBCONNECTION"
echo "MongoDB Database Name: $MYDBNAME"
echo "Env: $MYENV"

echo
echo "MongoDB Collections to be imported:"
ls -l ../mongodb-out-$MYENV/
echo
echo "START"
echo

CONN=$1
USER=$(echo $CONN | cut -c11- | cut -f 1 -d:)
PW=$(echo $CONN | cut -c11- | cut -f 2 -d: | cut -f 1 -d@)
MONGODB_HOST=$(echo $CONN | cut -c11- | cut -f 2 -d: | cut -f 2 -d@)
MONGODB_PORT=10255
...
for file in $(find ../mongodb-out-$MYENV/ -name '*.json'); do
  echo "importing $file collection in $MYDBNAME database"
  # If the collection already exists mongoimport will drop the existing collection first
  mongoimport -vvvvv -h $MONGODB_HOST:$MONGODB_PORT -d $MYDBNAME -u $USER -p $PW --ssl --jsonArray --file $file --writeConcern "{w:0}" --mode=merge
done

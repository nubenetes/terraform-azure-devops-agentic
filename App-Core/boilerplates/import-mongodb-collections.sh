...
echo "MONGODBCONNECTIONS: $MONGODBCONNECTIONS"
#echo "MYDBNAME: $MYDBNAME"
echo "MYDBADMIN: $MYDBADMIN"
#echo "MYSECRET: $MYSECRET"
echo "MYLISTOFCLIENTS: $MYLISTOFCLIENTS"
echo "MYEnterprisePRODUCT: $MYEnterprisePRODUCT"

echo
echo "MongoDB Collections to be imported:"
ls -la
echo
echo "START"
echo

MYEnterprisePRODUCT=$(echo $MYEnterprisePRODUCT | tr -d '"' )
MYLISTOFCLIENTS=$(echo $MYLISTOFCLIENTS | tr -d '[' | tr -d ']' | tr -d '"' )
# set comma as internal field separator for the string list
IFS=,
for value in ${MYLISTOFCLIENTS[@]};
do
    #database_name = "modb-${Enterprise_product}-$value"
    echo "modb-$MYEnterprisePRODUCT-$value"
done

# for connectionstring in $(echo $MONGODBCONNECTIONS | jq -r '.[]' | sed -e "s^//^//$MYDBADMIN:$MYSECRET@^" | tr -d '"'); do 
#   echo "MongoDB Connection: $connectionstring";
#   echo
#   for file in $(find . -name '*.json'); do
#     echo "importing $file collection in $connectionstring connection string"
#     #MYCOLLECTION=$(echo $file | cut -d'.' -f1) 
#     #mongoimport -vvvvv --db=$MYDBNAME --collection=$MYCOLLECTION --file $file $connectionstring
#     mongoimport -vvvvv --db=$MYDBNAME --file $file $connectionstring
#   done
# done
...
# --maintainInsertionOrder                    insert the documents in the order of their appearance in the input source. By default the insertions will be performed in an arbitrary order. Setting this flag also enables the behavior of --stopOnError and restricts NumInsertionWorkers to 1.

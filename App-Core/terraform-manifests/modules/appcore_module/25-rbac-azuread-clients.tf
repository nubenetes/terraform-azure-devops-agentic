...
# appr-appcore-back-<env> - Enterprise Application - Users and Groups
#       cg-appcore-<client_name>-admin_role-<env>
#       cg-appcore-<client_name>-user_role-<env>
#######################################################################
resource "azuread_app_role_assignment" "appcore_admin_back_api" {
  for_each            = toset(var.client_names)
  app_role_id         = azuread_service_principal.appcore_back_api.app_role_ids["Admin"]
  principal_object_id = azuread_group.appcore_admin_role[each.key].object_id
  resource_object_id  = azuread_service_principal.appcore_back_api.object_id
}
resource "azuread_app_role_assignment" "appcore_user_back_api" {
  for_each            = toset(var.client_names)
  app_role_id         = azuread_service_principal.appcore_back_api.app_role_ids["Viewer"]
  principal_object_id = azuread_group.appcore_user_role[each.key].object_id
  resource_object_id  = azuread_service_principal.appcore_back_api.object_id
}

...
if [ $# -ne 5 ]; then
  echo "please specify 5 command line arguments"
	exit 1
fi
...
echo "MongoDB Admin User: $MYDBADMIN"
echo "Env: $MYENV"
...
for file in $(find ../mongodb-out-$MYENV/ -name '*.json'); do
  echo "importing $file collection in $MYDBNAME database with $connectionstring connection string"
  mongoimport -vvvvv --db=$MYDBNAME --file $file $connectionstring --jsonArray
done

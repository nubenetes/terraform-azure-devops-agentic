...
resourceGroup="rg-catalog-myclient-dev-$randomIdentifier"
...
App-Catalog_agw="msdocs-app-gateway-$randomIdentifier"
...
az network application-gateway create --resource-group $resourceGroup --name $App-Catalog_agw --location "$location" --vnet-name $vNet --subnet $subnet --min-capacity 2 --sku Standard_v2 --http-settings-cookie-based-affinity Disabled --frontend-port 80 --http-settings-port 80 --http-settings-protocol Http --public-ip-address $publicIpAddress --servers $appFqdn

az network application-gateway http-settings update --resource-group $resourceGroup --gateway-name $App-Catalog_agw --name App-Catalog_agwBackendHttpSettings --host-name-from-backend-pool
...
az network public-ip show --resource-group $resourceGroup --name $publicIpAddress --query {App-Catalog_agwFqdn:dnsSettings.fqdn} --output table

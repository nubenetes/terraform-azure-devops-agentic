...
variable "dns_prefix" {
  ...
  type        = string
  default     = "ana" # Alternatives: qd, dis, appanalysis, etc
}
...
variable "Enterprise_product" {
  ...
  type        = string
  default     = "appanalysis" # Alternatives: qd, dis, appanalysis, etc
}
...
variable "secret_docker_registry_server_password" {
  description = "DOCKER_REGISTRY_SERVER_PASSWORD in 09-app-service.tf"
  sensitive   = true
}
variable "secret_app_analysis_config_key" {
  description = "config_key & Enterprise_key in 09-app-service.tf"
  sensitive   = true
}
...
variable "client_names" {
  ...
  validation {
    ...
    condition = alltrue([for each in var.client_names : (length("${each}") < 9) ? true : false])
  ...
  validation {
    condition     = alltrue([for each in var.client_names : can(regex("^[a-z0-9]+$", "${each}"))])
    error_message = "Please provide a valid value for variable client_names in lowercase with digits allowed"
  }
}
variable "client_names_with_enabled_app_gateways" {
  ...
  validation {
    ...
    # Storage account names must be between 3 and 24 characters in length and may contain numbers and lowercase letters only.
    # In consequence our client names cannot be longer than 8 char with the current naming scheme in our terraform code
    condition = alltrue([for each in var.client_names_with_enabled_app_gateways : (length("${each}") < 9) ? true : false])
    # cannot be longer than 8 char with the current keyvault naming scheme in our terraform code
    error_message = "Please provide a valid value for variable client_names with a maximum length of than 8 chars"
  }
  validation {
    condition     = alltrue([for each in var.client_names_with_enabled_app_gateways : can(regex("^[a-z0-9]+$", "${each}"))])
    error_message = "Please provide a valid value for variable client_names in lowercase with digits allowed"
  }
}
...
variable "mongodb_atlas_region" {
  ...
  type        = string
  default     = "EUROPE_NORTH" # August 2022: 409 (request "OUT_OF_CAPACITY") The requested region is currently out of capacity for the requested instance size.
  #default     = "EUROPE_WEST"
}
...
variable "k8s_cluster_name" {
  ...
  type        = string
  default     = "aks-computation-devtest" # "aks-computation-core"
}

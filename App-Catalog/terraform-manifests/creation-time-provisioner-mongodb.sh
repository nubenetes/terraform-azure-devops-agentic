...
if [ $# -ne 4 ]; then
  echo "please specify 4 command line arguments"
	exit 1
fi
...
connectionstring=$(echo $MONGODBCONNECTION | sed -e "s^//^//$MYDBADMIN:$MYSECRET@^" | tr -d '"')
echo "MongoDB Connection: $connectionstring";
echo
#for file in $(find enterprise_engine_database/data -name '*.json'); do
for file in $(find ../mongodb-out/ -name '*.json'); do
